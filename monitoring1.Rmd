...
banjir_bandang_j <- within(banjir_bandang_j, {TopoLegend <- ifelse(Topografi==1 , "Lereng", ifelse(Topografi==2, "Lembah", "Daratan"))})
```
...
```{r}
ggplot(banjir_bandang_j, aes(x = Prov, y = totalKorban)) +
  geom_bar(
...
ggplot(banjir_bandang_j, aes(x = Prov, y = totalKorban)) +
  ...
  facet_wrap(~SisTsunami) +
  labs(title="Jumlah korban bencana banjir bandang dengan peringatan Tsunami", x="Provinsi", y = "Korban Jiwa") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
  # theme_classic()
```
